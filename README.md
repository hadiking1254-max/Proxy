...
## ⚠️ قبل از شروع: چهار محدودیت واقعی Railway که باید بدانید
...
3. **"Limited Trial" ممکن است پورت TCP Proxy را اصلاً باز نکند.**
   حساب‌های Trial که با GitHub تأیید (verify) نشده باشند (اکانت
   GitHub خیلی جدید یا کم‌فعالیت) روی حالت **Limited Trial** قرار
   می‌گیرند که در آن ترافیک خروجی محدود است و **فقط تعداد کمی پورت**
   در دسترس است — این دقیقاً همان چیزی است که MTProto proxy به آن
   نیاز دارد. اگر بعد از فعال‌سازی TCP Proxy باز هم به پروکسی وصل
   نشدید، اول در Railway بررسی کنید که وضعیت حساب‌تان "Full Trial"
   است، نه "Limited" (با اتصال گیت‌هاب در railway.com/verify).
   راه‌حل قطعی در صورت رد شدن تأیید: ارتقا به پلن Hobby.
4. **گزینه‌ی "Serverless" (App Sleeping) را روشن نکنید.** این
   سرویس صرفاً بر اساس ترافیک **خروجی** تشخیص می‌دهد که سرویس بیکار
   است؛ یک پروکسی TCP که فقط منتظر اتصال کلاینت است هیچ ترافیک
   خروجی تولید نمی‌کند، پس بعد از ۱۰ دقیقه به خواب می‌رود و چون
   بیدارسازی خواب فقط با درخواست HTTP اتفاق می‌افتد نه یک اتصال TCP
   خام، کلاینت تلگرام دیگر هرگز نمی‌تواند آن را بیدار کند — یعنی
   پروکسی برای همیشه غیرقابل‌دسترس می‌ماند تا یک Redeploy دستی.
...
| بیلد با خطای `failed to resolve source metadata for ghcr.io/9seconds/mtg:vX.Y.Z: not found` شکست می‌خورد | نگه‌دارنده‌ی پروژه‌ی `mtg` فقط چند تگ اخیر را روی ghcr.io نگه می‌دارد؛ تگ پین‌شده در Dockerfile حذف (prune) شده | در [صفحه‌ی پکیج](https://github.com/9seconds/mtg/pkgs/container/mtg) آخرین تگ منتشرشده را ببین (فرمت آن بدون حرف `v` است، مثلاً `2.2.8` نه `v2.2.8`) و در خط `FROM ghcr.io/9seconds/mtg:...` در Dockerfile جایگزین کن |
